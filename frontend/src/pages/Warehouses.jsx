...
const Warehouses = () => {
    ...
    const addMutation = useMutation({
        ...
        onSuccess: () => { queryClient.invalidateQueries(['warehouses']); showToast('Anbar əlavə edildi'); setIsAddOpen(false); },
                    ...
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="rounded-3xl w-full max-w-md overflow-hidden shadow-2xl" style={{ backgroundColor: 'var(--color-card-bg)', border: '1px solid var(--color-card-border)' }}>
                            <div className="p-6 flex items-center justify-between" style={{ borderBottom: '1px solid var(--color-card-border)' }}>
                                <h3 className="text-xl font-black" style={{ color: 'var(--color-text-primary)' }}>Yeni Anbar</h3>
                                <button onClick={() => setIsAddOpen(false)}><X size={20} style={{ color: 'var(--color-text-muted)' }} /></button>
                            </div>
                            <form onSubmit={e => { e.preventDefault(); const fd = new FormData(e.target); addMutation.mutate({ name: fd.get('name'), address: fd.get('address') }); }} className="p-6 space-y-4">
                                <div>
                                    ...
                                    <input name="name" required className="w-full rounded-xl p-4 outline-none font-bold" style={{ backgroundColor: 'var(--color-input-bg)', border: '1px solid var(--color-input-border)', color: 'var(--color-text-primary)' }} placeholder="Əsas Anbar" />
                                </div>
                                <div>
                                    ...
                                    <input name="address" className="w-full rounded-xl p-4 outline-none font-bold" style={{ backgroundColor: 'var(--color-input-bg)', border: '1px solid var(--color-input-border)', color: 'var(--color-text-primary)' }} placeholder="Bakı, Nəsimi r." />
                                </div>
                                <button type="submit" disabled={addMutation.isPending} className="w-full py-4 rounded-xl font-black text-white" style={{ background: 'linear-gradient(135deg, #8b5cf6, #6d28d9)' }}>
                                    {addMutation.isPending ? 'Saxlanılır...' : 'Əlavə Et'}
                                </button>
                            </form>
                        </motion.div>
                    </div>
